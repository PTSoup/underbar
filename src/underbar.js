...
_.memoize = function(func) {
    ...
    return function() {
        ...
        if (args.length > 1) {
            return cache[args] ? cache[args] : func.apply(null, args);
        } else {
            return (cache[args] = (args in cache)) ? cache[args] : func.apply(null, args);
        }
...
_.shuffle = function(array) {
    ...
    // make an empty array
    // iterate thru the copy, and pull out numbers one by one, placing them into the empty array.
    var copy = array.slice();
    var output = [];
    var n = copy.length - 1;

    for (var i = n; n >= 0; n--) {
        var i = Math.floor(Math.random() * (Math.floor(n) - 0)) + 0;
